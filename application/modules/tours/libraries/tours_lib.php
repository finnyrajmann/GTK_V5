        ...
        if(!empty($date)){
        	$this->date = $date;
			   ...
			   //	$isfeatured = $this->is_featured();
                ...
                $detailResults = (object)array('id' => $details[0]->tour_id, 'title' => $title, 'slug' => $slug,'bookingSlug' => $bookingSlug,'thumbnail' => $thumbnail,
           		 'stars' => pt_create_stars($stars), 'starsCount' => $stars, 'location' => $location,'desc' => $desc,'inclusions' => $inclusions,'exclusions' => $exclusions,
           		 'latitude' => $latitude,'longitude' => $longitude,'sliderImages' => $sliderImages,'relatedItems' => $relatedTours,'paymentOptions' => $paymentOptions,
                  ...
                  'tourNights' => $tourNights,'totalDeposit' => $depositAmount,'mapAddress' => $details[0]->tour_mapaddress);
...
// Tour visiting Cities
		...
		public function siteMapData(){
         ...
         public function suggestionResults($query){
        	...
        	$tours[] =  (object)array('id' => '', 'name' => '', 'module' => 'tour', 'disabled' => true);
        	foreach($res as $r){
        		$title = $this->get_title($r->title,$r->id);
        		$tours[] =  (object)array('id' => $r->id, 'name' => trim($title), 'module' => 'tour', 'disabled' => false);  
        	}
